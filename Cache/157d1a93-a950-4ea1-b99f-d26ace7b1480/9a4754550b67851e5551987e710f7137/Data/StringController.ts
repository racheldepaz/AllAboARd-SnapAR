...
export class StringController extends BaseScriptComponent {
    ...
    updateBoatPosition(): void {
        ...
        if (this.isLeftEndHeld && this.isRightEndHeld) {
            ...
            // Here you would implement the logic to move the boat along the path
            // based on the midpoint position
            // This is a simplified version - you'll need to implement path following
            this.boat.getTransform().setWorldPosition(midpoint);
        }
    }
    
    onUpdate(): void {
        // Update the string visual every frame if both ends are held
        if (this.isLeftEndHeld || this.isRightEndHeld) {
            this.updateStringVisual();
        }
    }
