...
export class BoatController extends BaseScriptComponent {
    ...
    onStart(): void {
        ...
        const syncTransform = this.getSceneObject().getComponent("SyncTransform") as SyncTransform;
        if (!syncTransform) {
            console.warn("BoatController: No SyncTransform component found. Adding one...");
    ...
    initializePath(): void {
        ...
        if (!this.pathObject) {
            console.error("BoatController: No path object assigned");
            return;
        }
        ...
        if (this.pathPoints.length > 0) {
            ...
            if (this.pathPoints.length > 1) {
                const direction = vec3.normalize(vec3.sub(this.pathPoints[1], this.pathPoints[0]));
    ...
    getDirectionOnPath(t: number): vec3 {
        ...
        // Calculate direction
        return vec3.normalize(vec3.sub(pos2, pos1));
    }
