...
export class BoatController extends BaseScriptComponent {
    ...
    onUpdate(): void {
        ...
        if (vec3.length(direction) > 0.001) {
            const lookRotation = this.createLookRotation(direction, new vec3(0, 1, 0));
            this.getTransform().setLocalRotation(lookRotation);
        }
    }
    
    // Helper method to create a quaternion that looks in a specific direction
    private createLookRotation(forward: vec3, up: vec3 = new vec3(0, 1, 0)): quat {
        // Ensure forward vector is normalized
        forward = forward.normalize();
        
        // Make sure up and forward aren't parallel
        if (Math.abs(forward.dot(up)) > 0.999) {
            // If they're too close, slightly adjust the up vector
            up = Math.abs(forward.y) < 0.999 ? new vec3(0, 1, 0) : new vec3(1, 0, 0);
        }
        ...
        let q = new quat(0, 0, 0, 1); // Initialize with identity quaternion (w=1)
        ...
        // Normalize the quaternion manually since q.normalize() might return void
